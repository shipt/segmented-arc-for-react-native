      ...
      >
        {metaData => (
          <Pressable onPress={_handlePress} style={{ alignItems: 'center' }}>
            <Text style={{ fontSize: 16, paddingTop: 16 }}>{metaData.lastFilledSegment.passBackData.label}</Text>
...
| segments                    | Array of {scale: number, filledColor: string, emptyColor: string, passBackData: object } | []             | Segments of the arc. Here, scale is a percentage value out of 100%, filledColor for filled part of a segment, and emptyColor is background color for an empty segment, passBackData could be any object that you'd want to receive back for a segment. See example above. |
...
| children                    | function                                                                                 | '#FFFFFF'      | Pass a function as a child. It receives metaData with the last filled segment's data as an argument. From there you can extract passBackData object. See example above.                                                                                                   |
|                             |
